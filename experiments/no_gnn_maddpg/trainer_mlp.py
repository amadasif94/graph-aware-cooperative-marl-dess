"""
Trainer for no-GNN MADDPG DESS coordination.

This trainer reuses the same DESSEnv and GraphReplayBuffer as the graph-aware
experiments. The replay buffer still stores x, edge_index, and edge_attr, but
the no-GNN MADDPG model ignores them and uses only agent_obs.
"""
...
class MLPMADDPGTrainer:
    def __init__(
        ...
        self.noise_std = float(self.config.get("noise_std", 0.10))
        ...
        self.checkpoint_dir = Path(
            self.config.get(
                "checkpoint_dir",
                "results/checkpoints/maddpg_ieee33_mlp",
            )
        )
    ...
    def train(self):
        ...
        if self.verbose:
            print(
                f"Starting MLP-MADDPG training from global_step={self.global_step}, "
