"""
Evaluate trained no-GNN / MLP MADDPG policy on IEEE 69-bus DESS environment.
"""
...
def make_plots(step_df, episode_summary, figure_dir):
    ...
    mlp_df = df0[df0["policy"] == MLP_POLICY_NAME]

    if len(mlp_df) > 0:
        soc_cols = [
            col for col in mlp_df.columns
        ...
        for col in soc_cols:
            plt.plot(mlp_df["step"], mlp_df[col], label=col)
        ...
        plt.tight_layout()
        plt.savefig(
            figure_dir / f"soc_{MLP_POLICY_NAME}_episode_{first_episode}.png",
            dpi=300,
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            figure_dir / f"actions_{MLP_POLICY_NAME}_episode_{first_episode}.png",
            dpi=300,
        )
        plt.close()
...
def parse_args():
    ...
    parser.add_argument("--run_name", type=str, default="mlp_maddpg_ieee69_eval")
...
def main():
    ...
    print("===================================")
    print("No-GNN / MLP MADDPG IEEE69 Evaluation")
    print("===================================")
    print("Device:", device)
