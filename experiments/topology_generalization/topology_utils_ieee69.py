"""
Utilities for applying and validating IEEE 69-bus topology reconfiguration cases.

This module:
    - copies the base IEEE69 config,
...
def _find_edges_with_status_zero_based(line_df, edges, required_status, label):
    ...
    for edge in edges:
        ...
        if float(required_status) not in statuses:
            raise ValueError(
...
def build_topology_config(
    ...
    """
    Build a TP-specific IEEE69 config by writing temporary topology CSV files.

    Parameters
    ----------
    case_name : str
        TP case name, e.g., TP1, TP2, ..., TP9.

    base_config : dict or None
        Base config. If None, IEEE69_CONFIG is used.

    output_root : str or Path or None
        Directory where temporary topology files are written.
        If None:
            results/topology_generalization/generated_topologies_ieee69/<case_name>
    ...
    config = copy.deepcopy(IEEE69_CONFIG if base_config is None else base_config)
    ...
    config["case_name"] = "ieee69_{}".format(case_name)
...
def validate_all_topology_cases(case_names=None, base_config=None):
    """
    Validate all requested IEEE69 topology cases.
