"""
Evaluate trained graph-aware MADDPG policy under IEEE 33-bus topology
reconfiguration benchmark cases TP1-TP7.

Architecture compatibility:
    This script is compatible with the updated MADDPG model using:
...
Outputs:
    ...
    results/topology_generalization/csv/<run_name>/episode_summary.csv
    results/topology_generalization/csv/<run_name>/aggregate_summary.csv
    results/topology_generalization/figures/<run_name>/*.png
"""
...
from experiments.topology_generalization.topology_utils import (
...
def reset_at_episode_start(env, start_index, seed):
    """
    Reset environment at a fixed episode start index.
    """
...
def evaluate_one_episode(
    ...
    """
    Evaluate one episode under one topology and one policy.
    """
...
def make_plots(
    ...
    """
    Save topology generalization plots.
    """
...
def parse_args():
    ...
    parser.add_argument(
        "--run_name",
        type=str,
        default="tp_generalization_eval",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Topology cases to evaluate, e.g. TP1 TP2 TP3.",
    )

    parser.add_argument(
        "--csv_dir",
        type=str,
        default="results/topology_generalization/csv",
    )

    parser.add_argument(
        "--figure_dir",
        type=str,
        default="results/topology_generalization/figures",
    )
    ...
    # Training used share_actor=False.
    # Do not pass --share_actor unless the checkpoint was trained that way.
    parser.add_argument(
        "--share_actor",
        action="store_true",
    )
...
def main():
    """
    Main topology generalization evaluation routine.
    """
    ...
    if args.topologies is None:
        topology_cases = list_topology_cases(
            include_baseline=True
        )
    else:
        topology_cases = [
            tp.upper()
            for tp in args.topologies
        ]
    ...
    print("==============================================")
    print("Graph-Aware MADDPG Topology Generalization Eval")
    print("==============================================")
    print("Device:", device)
    ...
    for tp in topology_cases:
        ...
        if args.episodes == "all":
            start_indices = list(
                probe_env.episode_start_indices
            )
        else:
        ...
        print(
            "Description:",
            tp_config["topology_case"]["description"],
        )

        print(
            "Open edges:",
            tp_config["topology_case"]["open_edges"],
        )

        print(
            "Close edges:",
            tp_config["topology_case"]["close_edges"],
        )
        ...
        print(
            "In-service lines:",
            len(probe_env.grid.in_service_line_df),
        )

        print(
            "edge_index shape:",
            probe_env.grid.edge_index.shape,
        )
    ...
    print()
    print("Topology generalization evaluation complete.")
