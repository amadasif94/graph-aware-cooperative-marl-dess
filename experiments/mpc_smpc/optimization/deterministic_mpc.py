...
# ============================================================
# IEEE33 deterministic MPC using AutoGluon point forecasts.
#
# Main thesis baseline:
#   Deterministic MPC uses point forecasts:
#       forecast_load_node_i
#       forecast_pv_node_i
#
# from:
#   results/mpc_smpc/forecasts/forecast_33bus_load_pv_sep_dec_wide.csv
#
# It does NOT use SARIMA scenario mean/median as the main baseline.
#
# Solver reuse:
#   Uses stochastic_mpc.py with S=1.
#
# Topology support:
#   Passes topology_config/topology_case into stochastic_mpc.py so that
#   MPC first-action power-flow correction is consistent with TP1-TP7.
# ============================================================
...
from experiments.mpc_smpc.optimization.stochastic_mpc import (
...
class PointForecastData:
    ...
    forecast_load_kw: np.ndarray                 # [T, 33]
    forecast_pv_kw: np.ndarray                   # [T, 33]
    forecast_netload_kw: np.ndarray              # [T, 33]
...
def load_point_forecast_data(
    ...
    """
    Load December AutoGluon point forecasts and build aggregate net load.

    Expected forecast columns:
        forecast_load_node_1 ... forecast_load_node_33
        forecast_pv_node_1   ... forecast_pv_node_33

    Output:
        aggregate_netload_forecast_kw[t] =
            sum_buses(forecast_load_kw[t,b] - forecast_pv_kw[t,b])
    """
    ...
    # Keep December only if forecast_month exists.
...
def get_point_forecast_window(
    ...
    """
    Return deterministic point-forecast window.

    Shapes:
        aggregate_netload_kw : [T]
        load_kw              : [T, 33]
        pv_kw                : [T, 33]
        netload_kw           : [T, 33]
    """
...
def get_price_window_from_point_forecast(
    ...
    Priority:
        1. price from point forecast file
        2. fallback price array passed from scenario_data.py
...
def solve_deterministic_mpc(
    ...
    """
    Solve one deterministic MPC horizon using AutoGluon point forecast.

    Internally calls stochastic_mpc with S=1.

    If load_T33_kw and pv_T33_kw are provided, deterministic MPC also applies
    the same topology-specific first-action power-flow correction as SMPC.
    """
    ...
    if load_T33_kw is not None and pv_T33_kw is not None:
        load_T33_kw = np.asarray(load_T33_kw, dtype=np.float64)
        pv_T33_kw = np.asarray(pv_T33_kw, dtype=np.float64)

        expected_shape = (T, cfg.NUM_BUSES)

        if load_T33_kw.shape != expected_shape:
            raise ValueError(
                f"load_T33_kw shape {load_T33_kw.shape} != {expected_shape}"
            )

        if pv_T33_kw.shape != expected_shape:
            raise ValueError(
                f"pv_T33_kw shape {pv_T33_kw.shape} != {expected_shape}"
            )

        if not np.isfinite(load_T33_kw).all():
            raise ValueError("load_T33_kw contains NaN/inf.")

        if not np.isfinite(pv_T33_kw).all():
            raise ValueError("pv_T33_kw contains NaN/inf.")

        load_ST33_kw = load_T33_kw.reshape(1, T, cfg.NUM_BUSES)
        pv_ST33_kw = pv_T33_kw.reshape(1, T, cfg.NUM_BUSES)
...
def solve_first_action(
    ...
    """
    Rolling-horizon convenience wrapper.

    Uses point forecast window:
        aggregate_netload_forecast_kw[t0:t0+horizon]
    """
