# ============================================================
# scenario_data_ieee69.py
# ============================================================
# Load and validate IEEE69 SARIMA load/PV scenario tensors.
#
# Current SARIMA files contain:
#   scenarios_kw shape = [T_DEC, 69 buses, N_SCEN]
#
...
class ScenarioData:
    ...
    load_scenarios_kw: np.ndarray          # [T, 69, S]
    pv_scenarios_kw: np.ndarray            # [T, 69, S]
    netload_scenarios_kw: np.ndarray       # [T, 69, S]

    aggregate_load_scenarios_kw: np.ndarray
    aggregate_pv_scenarios_kw: np.ndarray
    aggregate_netload_scenarios_kw: np.ndarray
...
def _validate_tensor(
    ...
    if x.ndim != 3:
        raise ValueError(
            f"{label}: expected 3D tensor [T, buses, scenarios], got {x.shape}"
        )
...
def load_scenario_data(
    ...
    if not np.array_equal(load_bus_numbers, expected_bus_numbers):
        raise ValueError(
            f"Unexpected bus numbers. Expected {expected_bus_numbers}, "
            f"got {load_bus_numbers}"
        )
...
def get_aggregate_smpc_window(
    ...
    if t1 > data.aggregate_netload_scenarios_kw.shape[0]:
        raise ValueError(
            f"Window [{t0}:{t1}] exceeds available "
            f"T={data.aggregate_netload_scenarios_kw.shape[0]}"
        )
...
def get_nodal_smpc_window(
    ...
    return {
        "load_kw": np.transpose(
            data.load_scenarios_kw[t0:t1, :, :n_scenarios],
            (2, 0, 1),
        ).copy(),
        "pv_kw": np.transpose(
            data.pv_scenarios_kw[t0:t1, :, :n_scenarios],
            (2, 0, 1),
        ).copy(),
        "netload_kw": np.transpose(
            data.netload_scenarios_kw[t0:t1, :, :n_scenarios],
            (2, 0, 1),
        ).copy(),
    }
