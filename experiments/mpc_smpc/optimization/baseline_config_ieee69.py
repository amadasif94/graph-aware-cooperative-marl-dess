# ============================================================
# Optimization baseline configuration for IEEE69 MPC/SMPC
# ============================================================
...
SCENARIO_DIR = RESULTS_DIR / "scenarios_ieee69"
OPT_RESULTS_DIR = RESULTS_DIR / "optimization_ieee69"
...
LOAD_SCENARIO_FILE = SCENARIO_DIR / "load_scenarios_december_69bus.npz"
PV_SCENARIO_FILE = SCENARIO_DIR / "pv_scenarios_december_69bus.npz"

FORECAST_WIDE_FILE = FORECAST_DIR / "forecast_69bus_load_pv_sep_dec_wide.csv"
DECEMBER_FORECAST_FILE = FORECAST_DIR / "december_forecast_69bus_load_pv.csv"

SARIMA_MANIFEST_FILE = SCENARIO_DIR / "sarima_scenario_manifest_69bus.json"
...
# ============================================================
# IEEE69 / TIME SETTINGS
# ============================================================

NUM_BUSES = 69

# zero-based DESS buses:
# one-based reference: [14, 16, 18, 20, 22, 24, 26, 27, 65]
...
UNCONTROLLED_STEP_CSV = (
    UNCONTROLLED_RESULTS_DIR / "step_metrics_uncontrolled_ieee69.csv"
)

UNCONTROLLED_SUMMARY_CSV = (
    UNCONTROLLED_RESULTS_DIR / "summary_uncontrolled_ieee69.csv"
)

MPC_STEP_CSV = MPC_RESULTS_DIR / f"step_metrics_mpc_ieee69_h{HORIZON_T}.csv"
MPC_SUMMARY_CSV = MPC_RESULTS_DIR / f"summary_mpc_ieee69_h{HORIZON_T}.csv"

SMPC_STEP_CSV_TEMPLATE = str(
    SMPC_RESULTS_DIR
    / "step_metrics_smpc_ieee69_h{horizon}_S{scenarios}_alpha_{alpha}.csv"
)

SMPC_SUMMARY_CSV_TEMPLATE = str(
    SMPC_RESULTS_DIR
    / "summary_smpc_ieee69_h{horizon}_S{scenarios}_alpha_{alpha}.csv"
)
...
def get_mpc_output_paths(horizon_t, topology_case=DEFAULT_TOPOLOGY_CASE):
    ...
    step_path = out_dir / (
        f"step_metrics_mpc_ieee69_{topology_case}_h{horizon_t}.csv"
    )

    summary_path = out_dir / (
        f"summary_mpc_ieee69_{topology_case}_h{horizon_t}.csv"
    )
...
def get_smpc_output_paths(
    ...
    step_path = out_dir / (
        f"step_metrics_smpc_ieee69_{topology_case}_"
        f"h{horizon_t}_S{n_scen}_alpha_{tag}.csv"
    )

    summary_path = out_dir / (
        f"summary_smpc_ieee69_{topology_case}_"
        f"h{horizon_t}_S{n_scen}_alpha_{tag}.csv"
    )
...
def print_config_summary():
    print("=" * 72)
    print("IEEE69 MPC/SMPC OPTIMIZATION BASELINE CONFIG")
    print("=" * 72)
