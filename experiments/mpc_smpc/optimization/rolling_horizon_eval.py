...
def build_topology_env_config(topology_case: str = "TP1") -> dict:
    ...
    if topology_case == "TP1":
        return copy.deepcopy(IEEE33_CONFIG)

    return build_topology_config(topology_case)
...
def get_env_price_window(env: DESSEnv, current_index: int, horizon_t: int) -> np.ndarray:
    prices = []
...
def build_step_row(
    ...
    throughput_mwh = float(np.sum(np.abs(dess_power_agents))) * env.delta_t_hours / 1000.0
...
def summarize_results(step_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    episode_summary = (
        step_df
        .groupby(["topology_case", "controller", "episode_id", "start_index"], as_index=False)
        .agg(
            steps=("step", "count"),
    ...
    aggregate_summary = (
        episode_summary
        .groupby(["topology_case", "controller"], as_index=False)
        .agg(
...
def evaluate_one_episode(
    ...
    scenario_time_map = build_timestamp_index_map(scenario_data.timestamps) if scenario_data is not None else None
    point_time_map = build_timestamp_index_map(point_data.timestamps) if point_data is not None else None
    ...
    while not done:
        ...
        if controller == "smpc":
            ...
            scenario_time_index = resolve_time_index(current_date_time, scenario_time_map)
            ...
            scenario_time_index = resolve_time_index(current_date_time, point_time_map)
...
def _output_paths(
    ...
    if controller == "mpc":
        return (
            output_dir / f"step_metrics_mpc_{topology_case}_h{horizon_t}.csv",
            output_dir / f"episode_summary_mpc_{topology_case}_h{horizon_t}.csv",
            output_dir / f"aggregate_summary_mpc_{topology_case}_h{horizon_t}.csv",
        )

    return (
        output_dir / f"step_metrics_smpc_{topology_case}_h{horizon_t}_S{n_scenarios}.csv",
        output_dir / f"episode_summary_smpc_{topology_case}_h{horizon_t}_S{n_scenarios}.csv",
        output_dir / f"aggregate_summary_smpc_{topology_case}_h{horizon_t}_S{n_scenarios}.csv",
    )
...
def run_controller_evaluation(
    ...
    print("=" * 72)
    print("IEEE33 Optimization Baseline Evaluation")
    print("=" * 72)
    ...
    print()
    print("=" * 72)
    print("Saved optimization baseline outputs")
    print("=" * 72)
