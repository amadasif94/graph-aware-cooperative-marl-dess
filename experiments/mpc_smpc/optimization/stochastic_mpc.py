...
# stochastic_mpc.py
# ============================================================
# Network-aware IEEE33 multi-DESS SMPC.
#
# Uses nodal 33-bus load/PV scenarios:
#   load_kw[s, t, bus]
#   pv_kw[s, t, bus]
#
# Optimization:
#   - Gurobi MILP storage/grid dispatch
#   - first-step non-anticipativity
#   - SOC dynamics
#   - charge/discharge exclusivity
#   - grid import/export exclusivity
#
# Network feasibility:
#   - first action is evaluated through IEEE33 GridTensor power flow
#   - voltage and line-current feasibility are enforced by action scaling
#   - topology-specific config is used for TP1-TP7 consistency
#
# Note:
#   This is not a full nonlinear AC-OPF SMPC.
#   It is a forecast-driven SMPC with IEEE33 power-flow feasibility correction.
# ============================================================
...
def get_pf_env(
    ...
    """
    Return a topology-specific DESSEnv used only for power-flow checks.

    Important:
        The SMPC rolling environment may be TP2-TP7. Therefore, the
        internal PF-check environment must use the same topology config.
    """
    ...
    if key not in _PF_ENV_CACHE:
        config = copy.deepcopy(topology_config if topology_config is not None else IEEE33_CONFIG)

        _PF_ENV_CACHE[key] = DESSEnv(
            config=config,
            mode="test",
            seed=42,
        )

    return _PF_ENV_CACHE[key]


def clear_pf_env_cache() -> None:
    """
    Clear cached PF environments.

    Useful only if topology files/configs are changed within the same Python process.
    """
...
def apply_export_limited_curtailment(
    ...
    """
    Match MARL-style feeder export-limited curtailment.

    Positive DESS power = discharge/injection.
    Negative DESS power = charge/absorption.
    """
    ...
    dess_power_kw_full = np.asarray(dess_power_kw_full, dtype=np.float64).reshape(cfg.NUM_BUSES)
...
def check_first_action_network_feasible(
    load_S33_kw: np.ndarray,
    pv_S33_kw: np.ndarray,
    beta_ch_kw: np.ndarray,
    beta_dis_kw: np.ndarray,
    topology_config: Optional[dict] = None,
    topology_case: str = "TP1",
    ...
    """
    Check first-stage action across all scenarios using topology-specific IEEE33 power flow.
    """
    ...
    load_S33_kw = np.asarray(load_S33_kw, dtype=np.float64)
    pv_S33_kw = np.asarray(pv_S33_kw, dtype=np.float64)

    if load_S33_kw.ndim != 2 or load_S33_kw.shape[1] != cfg.NUM_BUSES:
        raise ValueError(
            f"load_S33_kw must have shape [S, {cfg.NUM_BUSES}], got {load_S33_kw.shape}"
        )

    if pv_S33_kw.ndim != 2 or pv_S33_kw.shape[1] != cfg.NUM_BUSES:
        raise ValueError(
            f"pv_S33_kw must have shape [S, {cfg.NUM_BUSES}], got {pv_S33_kw.shape}"
        )

    if load_S33_kw.shape != pv_S33_kw.shape:
        raise ValueError(
            f"load_S33_kw shape {load_S33_kw.shape} != pv_S33_kw shape {pv_S33_kw.shape}"
        )

    S = load_S33_kw.shape[0]
...
def correct_first_action_by_power_flow(
    ...
    """
    Scale first-step action toward zero until network-feasible.

    Same spirit as MARL hard feasibility correction.
    """
...
def solve_stochastic_mpc(
    ...
    del warm_start  # retained in signature for compatibility
    ...
    if load_ST33_kw is not None and pv_ST33_kw is not None:
        load_ST33_kw = np.asarray(load_ST33_kw, dtype=np.float64)
        pv_ST33_kw = np.asarray(pv_ST33_kw, dtype=np.float64)

        if load_ST33_kw.shape != (S, T, cfg.NUM_BUSES):
            raise ValueError(
                f"load_ST33_kw shape {load_ST33_kw.shape} != {(S, T, cfg.NUM_BUSES)}"
            )

        if pv_ST33_kw.shape != (S, T, cfg.NUM_BUSES):
            raise ValueError(
                f"pv_ST33_kw shape {pv_ST33_kw.shape} != {(S, T, cfg.NUM_BUSES)}"
            )

        aggregate_netload_ST_kw = np.sum(load_ST33_kw - pv_ST33_kw, axis=2)

    else:
        aggregate_netload_ST_kw = np.asarray(aggregate_netload_ST_kw, dtype=np.float64)

    if aggregate_netload_ST_kw.shape != (S, T):
        raise ValueError(
            f"aggregate_netload_ST_kw shape {aggregate_netload_ST_kw.shape} != {(S, T)}"
        )
    ...
    try:
        m = gp.Model(f"ieee33_network_aware_smpc_{topology_case}")
        ...
        import_big_m_kw = max(
            1.0,
            float(np.max(aggregate_netload_ST_kw)) + N * p_ch_max_kw + u_unmet_max_kw + 1000.0,
        )
    ...
    if enforce_network_first_action and load_ST33_kw is not None and pv_ST33_kw is not None:
        network_check = correct_first_action_by_power_flow(
            load_S33_kw=load_ST33_kw[:, 0, :],
            pv_S33_kw=pv_ST33_kw[:, 0, :],
...
def solve_first_action(
    load_ST33_kw: np.ndarray,
    pv_ST33_kw: np.ndarray,
    price_T: np.ndarray,
    soc0: np.ndarray,
    n_scenarios: int,
    horizon_t: int,
    ...
    return solve_stochastic_mpc(
        ...
        load_ST33_kw=load_ST33_kw,
        pv_ST33_kw=pv_ST33_kw,
        price_T=price_T,
        soc0=soc0,
