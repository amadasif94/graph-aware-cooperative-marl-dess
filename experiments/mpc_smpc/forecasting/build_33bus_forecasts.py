...
INPUT_PATH = (
    ...
    / "processed"
    / "mpc_smpc"
    / "ieee33_15min_nodal_load_pv_full_year.csv"
)

OUT_DIR = PROJECT_ROOT / "results" / "mpc_smpc" / "forecasts"
MODEL_DIR = PROJECT_ROOT / "results" / "mpc_smpc" / "models" / "autogluon_33bus"
...
def build_features_for_target(df: pd.DataFrame, target_col: str):
    ...
    # Cyclical time features for 15-min data
...
def prepare_ag_df(df_in: pd.DataFrame, feature_cols):
    out = df_in[[LABEL] + feature_cols].copy().reset_index(drop=True)
    return out


def run_forecast(args):
    print("=" * 80)
    print("IEEE33 WALK-FORWARD LOAD/PV FORECASTING")
    print("=" * 80)
    ...
    # Strict 15-min continuity check
    expected_index = pd.date_range(df.index.min(), df.index.max(), freq="15min")
    missing = expected_index.difference(df.index)
    if len(missing) > 0:
        raise ValueError(f"Missing 15-min timestamps detected. First missing: {missing[:5]}")
    ...
    for target_col in targets:
        ...
        for month_str in WF_MONTHS:
            ...
            print(f"  Train: {train_df.index.min()} -> {train_df.index.max()} | rows={len(train_df)}")
            print(f"  Test : {test_df.index.min()} -> {test_df.index.max()} | rows={len(test_df)}")
            ...
            if args.train_model:
                ...
                if not model_path.exists():
                    raise FileNotFoundError(
                        f"Saved model not found for {target_col}, {month_str}: {model_path}"
                    )
    ...
    all_preds_long = all_preds_long.sort_values(["target", DT_COL]).reset_index(drop=True)
    metrics_df = metrics_df.sort_values(["target", "forecast_month"]).reset_index(drop=True)

    all_long_path = OUT_DIR / "all_walkforward_predictions_sep_dec_long.csv"
    metrics_path = OUT_DIR / "monthly_metrics_sep_dec.csv"

    all_preds_long.to_csv(all_long_path, index=False)
    metrics_df.to_csv(metrics_path, index=False)

    # Wide forecast table for MPC/SMPC
    wide_parts = []
    ...
    for kind in ["load", "pv"]:
        ...
        yhat_wide = yhat_wide.rename(
            columns={c: f"forecast_{c}" for c in yhat_wide.columns if c not in [DT_COL, "forecast_month"]}
        )
        ytrue_wide = ytrue_wide.rename(
            columns={c: f"true_{c}" for c in ytrue_wide.columns if c not in [DT_COL, "forecast_month"]}
        )
        resid_wide = resid_wide.rename(
            columns={c: f"residual_{c}" for c in resid_wide.columns if c not in [DT_COL, "forecast_month"]}
        )
    ...
    wide_path = OUT_DIR / "forecast_33bus_load_pv_sep_dec_wide.csv"
    wide_df.to_csv(wide_path, index=False)

    residual_archive = wide_df[wide_df["forecast_month"].isin(RESIDUAL_MONTHS)].copy()
    december_forecast = wide_df[wide_df["forecast_month"] == SCENARIO_MONTH].copy()

    residual_path = OUT_DIR / "residual_archive_sep_nov_33bus_load_pv.csv"
    december_path = OUT_DIR / "december_forecast_33bus_load_pv.csv"
