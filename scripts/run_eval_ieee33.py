"""
Evaluate trained graph-aware MADDPG policy on IEEE 33-bus DESS environment.

Architecture compatibility:
    This evaluation script is compatible with the updated MADDPG model using:
        - decentralized graph-aware actors
        - agent-specific centralized critics

Outputs:
    results/csv/<run_name>/step_metrics.csv
    results/csv/<run_name>/episode_summary.csv
    results/csv/<run_name>/aggregate_summary.csv
...
def build_agent(env, args, device):
    """
    Build graph-aware MADDPG agent and load trained checkpoint.
    """
...
def reset_at_episode_start(env, start_index, seed):
    """
    Reset environment at a specific episode start index.
    """
...
def get_action(policy, agent, env, obs):
    """
    Return action for selected policy.
    """
...
def evaluate_one_episode(env, agent, policy, episode_id, start_index, seed):
    """
    Evaluate one full episode and return per-step rows.
    """
    ...
    while not done:
        ...
        next_obs, reward, terminated, truncated, info = env.step(requested_action)
        done = bool(terminated or truncated)

        reward = np.asarray(reward, dtype=np.float64)
        kpis = info["kpis"]

        accepted_action = np.asarray(info["accepted_action"], dtype=np.float64)
        requested_action = np.asarray(info["requested_action"], dtype=np.float64)
        ...
        soc_values = [float(battery.get_soc()) for battery in env.batteries]
...
def summarize(step_df):
    """
    Build episode-level and aggregate-level summaries.
    """

    episode_summary = step_df.groupby(
        ["policy", "episode_id", "start_index"]
    ).agg(
    ...
    aggregate_summary = episode_summary.groupby("policy").agg(
        ...
        mean_infeasible_requested_count=("infeasible_requested_count", "mean"),
...
def make_plots(step_df, episode_summary, figure_dir):
    ...
    for metric in episode_metrics:
        plt.figure()
        episode_summary.boxplot(column=metric, by="policy")
        plt.title(metric)
        plt.suptitle("")
        plt.ylabel(metric)
        plt.tight_layout()
        plt.savefig(figure_dir / f"{metric}_boxplot.png", dpi=300)
        plt.close()

    first_episode = int(step_df["episode_id"].min())
    df0 = step_df[step_df["episode_id"] == first_episode]
    ...
    for metric in time_metrics:
        ...
        for policy, group in df0.groupby("policy"):
            plt.plot(group["step"], group[metric], label=policy)
        ...
        plt.tight_layout()
        plt.savefig(
            figure_dir / f"{metric}_episode_{first_episode}.png",
            dpi=300,
        )
        plt.close()

    maddpg_df = df0[df0["policy"] == "maddpg"]

    if len(maddpg_df) > 0:
        soc_cols = [
            col for col in maddpg_df.columns
            if col.startswith("soc_agent_")
        ]

        plt.figure()

        for col in soc_cols:
            plt.plot(maddpg_df["step"], maddpg_df[col], label=col)
        ...
        plt.tight_layout()
        plt.savefig(
            figure_dir / f"soc_maddpg_episode_{first_episode}.png",
            dpi=300,
        )
        plt.close()
        ...
        action_cols = [
            col for col in maddpg_df.columns
            if col.startswith("accepted_action_agent_")
        ]
        ...
        plt.xlabel("Step")
        plt.ylabel("Accepted action")
        plt.legend()
        plt.tight_layout()
        plt.savefig(
            figure_dir / f"actions_maddpg_episode_{first_episode}.png",
            dpi=300,
        )
        plt.close()
...
def parse_args():
    ...
    parser.add_argument("--run_name", type=str, default="maddpg_eval")
    ...
    # Default remains False unless explicitly passed.
...
def main():
    ...
    if args.episodes == "all":
        ...
        start_indices = list(env.episode_start_indices[:num_episodes])
    ...
    print("===================================")
    print("Graph-Aware MADDPG IEEE33 Evaluation")
    print("===================================")
    print("Device:", device)
    ...
    for policy in policies:
        for ep_id, start_idx in enumerate(start_indices):
            ...
            ep_reward_mean = sum(row["reward_mean"] for row in rows)
            ep_reward_team = sum(row["reward_sum"] for row in rows)
            ep_cost = sum(row["energy_cost"] for row in rows)
            ep_feasible_rate = np.mean([row["feasible"] for row in rows])
