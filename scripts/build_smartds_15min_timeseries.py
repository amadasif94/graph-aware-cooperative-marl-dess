"""
Build 15-minute IEEE33 node-level time-series data from SMART-DS load/PV
...
OUT_PATH = PROJECT_ROOT / "data" / "time_series" / "processed_15min_smartds.csv"
...
# Scale SMART-DS building loads to match IEEE-33 feeder loading.
LOAD_SCALING_FACTOR = 0.68

# Each SMART-DS solar CSV is for a 1000 kW reference PV array.
# 0.20 converts each profile to an effective 100 kW PV system.
PV_SCALING_FACTOR = 0.2
...
def find_price_column(df):
    ...
    raise ValueError(f"Could not identify price column. Columns: {list(df.columns)}")
...
def read_load_file(path):
    ...
    if "total_site_electricity_kw" not in df.columns:
        raise ValueError(f"{path.name} missing total_site_electricity_kw column.")

    if len(df) != N_15MIN_STEPS:
        raise ValueError(f"{path.name} has {len(df)} rows, expected {N_15MIN_STEPS}.")
...
def read_solar_file(path):
    ...
    if pv_col not in df.columns:
        raise ValueError(f"{path.name} missing PV column: {pv_col}")
...
def read_price_file(path, master_index):
    ...
    if len(price_15min) < N_15MIN_STEPS:
        raise ValueError(
            f"Price file gives only {len(price_15min)} 15-min values, "
            f"expected at least {N_15MIN_STEPS}."
        )

    price_15min = price_15min.iloc[:N_15MIN_STEPS].reset_index(drop=True)
...
def aggregate_profiles_to_buses(files, reader_func, num_files, files_per_bus, prefix, master_index):
    files = sorted(files)[:num_files]

    if len(files) < num_files:
        raise ValueError(f"Need {num_files} files for {prefix}, found {len(files)}.")
    ...
    for bus_idx in range(NUM_BUSES):
        selected = files[
            bus_idx * files_per_bus : (bus_idx + 1) * files_per_bus
        ]

        total = pd.Series(0.0, index=range(N_15MIN_STEPS))
...
def main():
    print("===================================================")
    print("SMART-DS 15-MIN PREPROCESSING")
    print("===================================================")

    if NUM_LOAD_FILES % NUM_BUSES != 0:
        raise ValueError("NUM_LOAD_FILES must be divisible by NUM_BUSES.")

    if NUM_SOLAR_FILES % NUM_BUSES != 0:
        raise ValueError("NUM_SOLAR_FILES must be divisible by NUM_BUSES.")
    ...
    price = read_price_file(PRICE_PATH, master_index)

    print("Merging final dataset...")
    df = load_bus.merge(solar_bus, on="date_time", how="inner")
    df = df.merge(price, on="date_time", how="inner")

    if len(df) != N_15MIN_STEPS:
        raise ValueError(f"Final dataframe has {len(df)} rows, expected {N_15MIN_STEPS}.")

    ordered_cols = ["date_time"]

    ordered_cols += [
        f"active_power_node_{i}" for i in range(1, NUM_BUSES + 1)
    ]

    ordered_cols += [
        f"renewable_active_power_node_{i}" for i in range(1, NUM_BUSES + 1)
    ]
    ...
    df["date_time"] = df["date_time"].dt.strftime("%Y-%m-%d %H:%M:%S%z")
    ...
    load_cols = [f"active_power_node_{i}" for i in range(1, NUM_BUSES + 1)]
    pv_cols = [f"renewable_active_power_node_{i}" for i in range(1, NUM_BUSES + 1)]
