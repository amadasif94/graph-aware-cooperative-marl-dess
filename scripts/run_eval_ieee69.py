"""
Evaluate trained graph-aware MADDPG policy on IEEE 69-bus DESS environment.
...
def evaluate_one_episode(env, agent, policy, episode_id, start_index, seed):
    ...
    while not done:
        ...
        for i in range(env.num_agents):
            row[f"requested_action_agent_{i}"] = float(
                requested_action[i]
            )

            row[f"accepted_action_agent_{i}"] = float(
                accepted_action[i]
            )

            row[f"dess_power_kw_agent_{i}"] = float(
                dess_power_agents[i]
            )

            row[f"soc_agent_{i}"] = float(
                soc_values[i]
            )
...
def make_plots(step_df, episode_summary, figure_dir):
    ...
    df0 = step_df[
        step_df["episode_id"] == first_episode
    ]
...
def main():
    ...
    if args.episodes == "all":
        start_indices = list(
            env.episode_start_indices
        )
    else:
        num_episodes = int(args.episodes)

        start_indices = list(
            env.episode_start_indices[:num_episodes]
        )
    ...
    print("===================================")
    print("Graph-Aware MADDPG IEEE69 Evaluation")
    print("===================================")
    ...
    episode_summary, aggregate_summary = summarize(
        step_df
    )
